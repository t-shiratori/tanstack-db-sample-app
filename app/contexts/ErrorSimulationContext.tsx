...
const ErrorSimulationContext = createContext<
	ErrorSimulationContextType | undefined
>(undefined);
...
export function ErrorSimulationProvider({ children }: { children: ReactNode }) {
	...
	return (
		<ErrorSimulationContext.Provider
			value={{ simulateError, setSimulateError: handleSetSimulateError }}
		>
			{children}
		</ErrorSimulationContext.Provider>
	);
...
export function useErrorSimulation() {
	...
	if (!context) {
		throw new Error(
			"useErrorSimulation must be used within ErrorSimulationProvider",
		);
	}
	return context;
}
